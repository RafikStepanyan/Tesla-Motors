...
export const AddManager: React.FC = (): JSX.Element => {
    const { register, handleSubmit, reset, formState: { errors } } = useForm();

    const save = (data: any): void => {
        createUserWithEmailAndPassword(auth, data.email, data.password).then(res => {
            delete data.password;
            addData({ collectionName: 'user', obj: { ...data, type: 'manager', blocked: false } }).then(res => console.log(res)).catch(res => console.log(res));
    ...
    return (
        <div className='login'>
            <form style={{ width: '400px', height: '520px', overflowY: 'auto' }} onSubmit={handleSubmit(save)}>
                <h1 style={{ fontSize: '50px', fontWeight: '500' }}>Create Manager</h1>
                <label>Name</label>
